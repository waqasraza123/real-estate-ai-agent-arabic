...
- The worker now also runs a persisted `CaseAgentOrchestrator` loop for `new_lead`, `no_response_follow_up`, `document_missing`, and `inbound_customer_message` triggers, with typed run records, compact case-agent memory, explicit manager-escalation boundaries, and WhatsApp transport delegated through the lower-level outbound queue
...
- Inbound WhatsApp messages are now first-class agent input instead of passive timeline events: webhook intake can switch the case locale to Arabic when needed, open policy-driven QA holds from inbound content, and queue an `inbound_customer_message` agent cycle so the worker can reply, draft, or block based on the latest customer message
- Case-agent memory is now structured instead of summary-only: persisted memory includes intent category, requested next step, urgency, customer sentiment, and objection categories, and the deterministic/runtime/model paths all use that structure to shape bilingual replies, draft downgrades, and escalation decisions
- Outbound case-agent decisions now carry commercial fact grounding: approved bilingual policy facts are persisted locally, required fact kinds are retrieved before each agent run, commitment-bearing WhatsApp sends are downgraded or escalated when evidence is missing or unsafe, and lead/conversation surfaces expose the latest grounding state
...
- The first provider-ready decision-model upgrade is now live locally, document intelligence over uploaded evidence now includes persisted extraction provenance and optional OCR, and inbound WhatsApp is now part of the same agent runtime, so the next step should be stronger live-model rollout, richer bilingual objection handling, and broader exception-eval coverage rather than more basic workflow plumbing
- The next production-grade step after structured conversation intelligence should be grounding the agent on approved commercial facts and pricing sources before outbound sends, so commitment-bearing replies rely on explicit retrieved evidence instead of prompt-only policy
- The first approved-commercial-fact grounding slice is now live locally; the next depth step should be replacing seeded policy facts with client-managed source lifecycle, per-project sales sheets, expiry review, and stronger eval coverage for evidence-required replies
...
- Added the first commercial fact-grounding slice with approved bilingual fact persistence, case-agent run grounding metadata, model prompt grounding context, send-time downgrade or escalation for missing/unsafe commercial evidence, lead/conversation visibility, and detailed implementation documentation
